...
**Status: phase 1 code-complete (2026-08-13)**, delivered by the Android track as Sprint A5 (`docs/android-client-plan.md`). Everything above except the Motzei reconciliation sheet is built: zmanim computed in the Worker and published as synced `ShabbatWindow` records, the `weekday → pre_shabbat_arming → shabbat_active → motzei_pending` predicates in `packages/shared/src/shabbat.ts` that every surface reads, Shabbat-kind pushes with no actions, no escalation and no missed sweep in mode, `pending_shabbat` logs written by the Durable Object, the web burst as a chain-driven schedule, the Shabbat channel on Android, and the 8-week verification screen on both clients. **The reconciliation sheet is phase 2** — the logs it consumes exist now, and until it ships both Today views show those doses as awaiting reconciliation rather than done. Windows are server-authored: the sync push route rejects a client-written one, because a device that could mint windows could move every other device into or out of Shabbat mode.
