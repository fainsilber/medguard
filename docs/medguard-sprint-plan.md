...
**Progress:** Sprints 0–4 complete, deployed, and verified against the live API. **Sprint 5 (alarms, Web Push & escalation) is code-complete as of 2026-08-09**, delivered by the Android track as Sprint A4 (`docs/android-client-plan.md`) rather than built standalone on web first: the Durable Object dose-alarm chain, escalation at the household's configured boundary, the missed-dose sweep, low-stock push, an FCM sender alongside the existing VAPID one, and both clients' receive paths. Its `runDurableObjectAlarm` exit gate passes; the two-real-phone half is still yours to run, alongside the Sprints 3–4 two-phone check. **Sprint 6 / A5 (Shabbat & Yom Tov) is code-complete as of 2026-08-13** — zmanim, mode, alarms, the verification screen and the Motzei reconciliation sheet. It is built against the *placeholder* halachic answers in `docs/halachic-decisions.md`, and its remaining exit gate is yours: check 8 weeks of times on the Shabbat screen against your luach.
...
**D5 — Shabbat pushes must carry no action buttons, and reach every caregiver at once instead of escalating.** Standard-mode pushes have "Taken"/"Snooze" lock-screen buttons; tapping one on Shabbat writes data. **Fix:** informational-only pushes while in mode. Reconciliation happens after Havdalah. Per `docs/halachic-decisions.md` Q3, escalation isn't simply turned off — the dose-time burst fans out to *every* registered device in the household on the first push, so there is structurally nothing left to escalate to. This differs from standard mode (Sprint 5), where escalation reaches everyone after 15 minutes unacknowledged. *As built in A4, standard mode's initial dose push also goes to every registered device — the server cannot know which phone a caregiver is actually near, and for a medication alarm one redundant notification is the correct direction to fail. What escalation adds is the `dose_escalation_v1` channel: DND bypass where granted, and a full-screen intent where the platform permits one. The Shabbat distinction is therefore about the channel and the absence of actions, not about who is reached.*
...
### Sprint 4 — Durable Objects: real-time sync & the double-dose guard — ✅ **Complete** (2026-08-04)

**Delivered:** `HouseholdDO` WebSocket Hibernation channel (`GET /api/v1/live`) with broadcast fan-out; every sync push now runs through `HouseholdDO.applyBatch` rather than writing to D1 directly from the Worker, which is what makes the DO's single-threaded processing into the actual serialization point delta D2 needs; the client sync engine (outbox drain, cursor-based pull with LWW merge, bootstrap on first connect); the live WebSocket client with deterministic exponential-backoff reconnect; a sync-status indicator and a safety-warning banner (invariant 6). 700 tests green; coverage gates hold, including `sync.ts` at 100% branch; 15 Playwright tests including two real-time cross-device specs against a real local API.

**Decisions worth knowing:**
- **All sync writes route through the DO, not just intake logs.** Simpler than special-casing "only safety-critical tables go through the DO" — the whole push batch gets the same serialization and the same one-shot broadcast at the end, and `HouseholdDO.applyBatch` reuses the exact same per-table write logic (`apps/api/src/sync/repository.ts`) the Worker used to call directly.
- **The safety re-check evaluates "as of the dose's own `actualTime`", not the server's current clock.** Reusing `assessDose` with a `Clock` that reports the incoming record's own `actualTime` as "now" is what correctly answers "would this dose have been permitted given everything already accepted before it" — the actual question a race needs answered — rather than "is a dose permitted right now", which doesn't handle two near-simultaneous submissions arriving moments apart.
- **A dose blocked by the safety check must exclude its own id from its own comparison history**, or a resent/duplicate push of an already-applied log sees itself in the history and gets blocked as a cooldown violation against itself. Caught by an integration test, not by design — worth flagging since it is exactly the kind of bug that only shows up once idempotent retry and the authoritative safety check exist in the same code path.
- **A blocked inventory adjustment closes a real D3 gap**: if the log it decrements stock for was blocked (in the same batch or an earlier one), the adjustment is blocked too — otherwise a rejected dose could still have silently decremented stock.
- **`/live` is its own router at `/api/v1/live`, not nested under `/api/v1/sync`.** Hono merges a sub-app's `.use('*', middleware)` into the parent's routing table for its whole mount prefix, so nesting it under `syncRoutes` would have put the WebSocket upgrade behind that router's blanket `requireDevice` — which only reads an `Authorization` header, and a browser's native WebSocket API cannot set one. The device token instead travels as the `Sec-WebSocket-Protocol` value.
- **The push response's `blocked` array replaces the PRD-era plan's single `409 SAFETY_VIOLATION`.** The batched multi-table push endpoint Sprint 3 actually built has no single verdict for a whole request — each record already gets its own outcome (`applied` / `duplicate` / `superseded` / `rejected`), so `blocked` is one more entry in that same per-record shape rather than a special-cased HTTP status.
- **The client had no way to push a new local mutation promptly** until an explicit fix mid-sprint: the sync engine originally drained the outbox only on mount, on WebSocket open, or on an incoming broadcast — nothing told it "a caregiver's own dose was just logged, go push it." A Dexie live query watching `syncOutbox`'s count closes that gap.

**Tests:** the race test (two concurrent PRN pushes for the same medicine, real workerd + real D1, asserting exactly one accepted); WebSocket fan-out to every connected device; cross-household isolation on the live channel; the orphan-adjustment guard; client engine tests for outbox drain and pull-merge (including the LWW pending-edit protection); `LiveClient` reconnect/backoff under fake timers; two Playwright specs against a real local `wrangler dev` — cross-device propagation measured under the PRD's 1.5s budget, and offline-logged-then-replayed with zero log loss verified by a third device joining fresh afterwards.

**Live-verified** against local `wrangler dev` + real local D1: created a household, added a medicine, gave a dose, watched the sync badge go Synced within moments (confirming the mid-sprint immediate-drain fix); then, via direct API calls simulating two devices, confirmed an unguarded medicine allows two concurrent doses (correct — no guard configured) and a guarded one blocks the second with a real `cooldown` verdict and `availableAtIso`.

**Still needs you:** the two-real-phone check — the race test and the 1.5s budget are proven in workerd and in a real local browser/API pair, but not yet over real cellular between two physical devices.
...
**Known risk — spike this first.** The standard `web-push` npm package depends on Node crypto and does not run on workerd. VAPID JWT signing and aes128gcm encryption must use WebCrypto (`webpush-webcrypto`, or ~150 lines hand-rolled). This is the single most likely thing to slip. *Resolved: hand-rolled in Sprint 0 (`apps/api/src/push/encrypt.ts`, verified against RFC 8291's worked example) and the same approach carried A4's FCM service-account signing (`fcm.ts`) without trouble.*

**Delivered by Sprint A4 (2026-08-09), code-complete.** What actually shipped, and where it differs from the scope above: escalation and dose scheduling live in `HouseholdDO` via `apps/api/src/do/doseAlarms.ts`, driven by a 48-hour horizon re-materialized from `schedules` on every relevant write — so a schedule edit cancels and reschedules implicitly rather than through per-schedule bookkeeping. Acknowledgement needs no new channel: an `IntakeLog` or `DoseSnooze` landing in `applyBatch` is what stops the chain, which is also why `DoseSnooze` had to become a synced entity (delta AD5, shipped in A3). "Missed-dose detection" is pinned to a definition in delta AD6 and written as a real, correctable `IntakeLog`. The **Web Worker timer engine and the HTML5 Audio chime were not built**: the native client plays the real 45-second chime from a local alarm (the entire reason it exists — delta D1), and building a browser approximation of it now would be work aimed at a client that is being migrated away from on Android (delta AD9). The web keeps the push burst it already has.
...
**Exit gate:** escalation fires at exactly the configured boundary (**passing** — `apps/api/tests/alarms.test.ts` asserts the exact instant the chain arms for, under `runDurableObjectAlarm`); a real push lands on a real Android device (**not yet run** — needs a Firebase project and the `FCM_SERVICE_ACCOUNT` Worker secret).
...
**Status: code-complete (2026-08-13)**, delivered by the Android track as Sprint A5 (`docs/android-client-plan.md`) in two phases. Phase 1 — mode and alarms: zmanim computed in the Worker and published as synced `ShabbatWindow` records, the `weekday → pre_shabbat_arming → shabbat_active → motzei_pending` predicates in `packages/shared/src/shabbat.ts` that every surface reads, Shabbat-kind pushes with no actions, no escalation and no missed sweep in mode, `pending_shabbat` logs written by the Durable Object, the web burst as a chain-driven schedule, the Shabbat channel on Android, and the 8-week verification screen on both clients. Phase 2 — the Motzei Shabbat sheet: it opens itself after Havdalah, bulk-confirms in one transaction, takes per-item answers, accepts retroactive PRN entry judged as of when the dose was given, and refuses a second caregiver's answer to a dose already settled (`already_superseded`, checked in the Durable Object where the question has a single answer). Windows are server-authored throughout: the sync push route rejects a client-written one, because a device that could mint windows could move every other device into or out of Shabbat mode.
...
**Exit gate:** fixtures match published times (**passing** — `apps/api/tests/zmanim.test.ts`, including a DST-boundary Shabbat and the Jerusalem-custom trap described in the A5 notes); three-day chag continuity green (**passing**); suppression tests green (**passing** — `apps/api/tests/shabbatAlarms.test.ts`); reconciliation E2E green (**passing** — `apps/web/e2e/motzei-reconciliation.spec.ts`, against the real API); **you have verified 8 weeks against your luach** — wrong by 18 minutes is a real problem — **not yet done**, and it is the half of this gate the machine cannot cover.
