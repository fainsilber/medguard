 ...
 * opens a real `expo-sqlite` database asynchronously and renders a loading spinner (with no
 * `testID` of its own — it's the composition root, out of scope for this sprint's features to
 * change) until its handles resolve, so every caller must `waitFor` past that itself — typically
 * by asserting on something the screen under test renders once mounted (its title, an empty-state
 * string, …). This helper only centralizes the provider wiring, not that wait.
 *
