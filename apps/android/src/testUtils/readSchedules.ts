 ...
 * component wrote through, so schedule-form tests can assert what actually landed in storage —
 * not just which internal branch a component took. Read-only helpers here don't care about their
 * own clock/ids.
 */
...
export async function readSchedules(dbName: string, medicineId: string): Promise<Schedule[]> {
  const repository = await repositoryFor(dbName);
  return repository.schedulesForMedicine(medicineId);
}

/** Pre-seeds a schedule before a component under test mounts against the same `dbName`. */
export async function seedSchedule(dbName: string, schedule: Schedule): Promise<void> {
  const repository = await repositoryFor(dbName);
