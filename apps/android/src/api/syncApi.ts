...
/**
 * The sync surface — bootstrap, cursor-based pull, batched push. See
 * `apps/api/src/routes/sync.ts`. Ported verbatim from `apps/web/src/api/syncApi.ts`: this is
 * plain `fetch` through `householdApi.ts`'s `request()`, which works unchanged under RN/Metro.
 */
