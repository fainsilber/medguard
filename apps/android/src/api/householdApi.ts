...
/**
 * The household, join-code, and device-lifecycle endpoints — ported near-verbatim from
 * `apps/web/src/api/householdApi.ts`. React Native's global `fetch` is API-compatible, so
 * `request()` is unchanged; the only real difference is `platformDescription()` below, which
 * replaces web's `navigator.userAgent` (not meaningfully populated in React Native) with
 * `expo-device`'s model/OS fields.
 *
...
/** e.g. "Pixel 8 (Android 15)" — the Android equivalent of web's `navigator.userAgent.slice(0, 50)`. */
function platformDescription(): string {
  const model = Device.modelName ?? 'Android device';
  const os = Device.osVersion ? ` (Android ${Device.osVersion})` : '';
  return `${model}${os}`.slice(0, 50);
}
...
export function createHousehold(
  ...
  return request('POST', `${apiBaseUrl}/api/v1/households`, {
    body: { ...input, platform: platformDescription() },
  });
}
...
export function redeemJoinCode(
  ...
  return request('POST', `${apiBaseUrl}/api/v1/join-codes/redeem`, {
    body: { ...input, platform: platformDescription() },
  });
}
 ...
 * local sign-out can't do. Without this, a token that was somehow retained elsewhere would stay
 * valid indefinitely.
 */
